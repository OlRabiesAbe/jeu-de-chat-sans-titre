...
function StartBtn(game, x, y) {
	this.animation = new Animation(ASSET_MANAGER.getAsset("./img/start_btn.png"), 0, 0, 359, 152, 1, 1, true, false);
	this.hoverAnim = new Animation(ASSET_MANAGER.getAsset("./img/start_btn.png"), 0, 153, 359, 152, 1, 1, true, false);
	this.hover = false;
	Entity.call(this, game, x, y);
}
...
// This button is on the title screen and will start the game
function WinBtn(game, x, y) {
	this.animation = new Animation(ASSET_MANAGER.getAsset("./img/win_btn.png"), 0, 0, 359, 152, 1, 1, true, false);
	this.hoverAnim = new Animation(ASSET_MANAGER.getAsset("./img/win_btn.png"), 0, 153, 359, 152, 1, 1, true, false);
	this.hover = false;
	Entity.call(this, game, x, y);
}

WinBtn.prototype = new Entity();
WinBtn.prototype.constructor = WinBtn;

WinBtn.prototype.update = function() {
	this.hover = this.game.winBtnHover;
	if (this.game.click && this.hover && !this.removeFromWorld) {
...
WinBtn.prototype.draw = function(ctx) {
	if (!this.hover) {
		this.animation.drawFrame(this.game.clockTick, ctx, this.x, this.y);
	} else {
		this.hoverAnim.drawFrame(this.game.clockTick, ctx, this.x, this.y);
